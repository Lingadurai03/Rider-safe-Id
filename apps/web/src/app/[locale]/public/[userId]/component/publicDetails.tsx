...
const PublicDetails = async ({ userId }: { userId: string }) => {
    ...
    try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL_SSR}public/${userId}`);
    ...
    if (!qrProfileData) return <p className="text-center">{t('noProfileDataFound')}</p>;
    return (
        <div className="max-w-4xl mx-auto bg-white-sm backdrop-blur-sm rounded-xl shadow-lg p-6 space-y-8 border border-gray-200">
            ...
            <div className="text-center space-y-3">
                <h2 className="font-bold text-3xl text-primary">{qrProfileData.profileName}</h2>
                <p className="text-gray-600">
                    {t('bloodGroup')}:{' '}
                    <span className="font-semibold text-black">{qrProfileData.bloodGroup}</span>
                </p>
                <p className="text-gray-600">
